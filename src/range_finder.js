...
class RangeFinder {
    constructor(maxSpeed, stepSize) {
        ...
        for (let launchAngle = MIN_LAUNCH_ANGLE; launchAngle <= MAX_LAUNCH_ANGLE; launchAngle += stepSize) {
            for (let speed = MIN_SPEED; speed <= maxSpeed; speed += stepSize) {
                for (let angleOfAttack = MIN_ANGLE_OF_ATTACK; angleOfAttack <= MAX_ANGLE_OF_ATTACK; angleOfAttack += stepSize) {
